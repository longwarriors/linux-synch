...
############################
cp.random.seed(1)
A_in_d = cp.random.random(extent_A_in, dtype=np.float32).astype(
    np.float32, order="F"
)  # we use fortran layout throughout this example
...
strides = 0
desc_tensor_A_in = cutn.create_tensor_descriptor(
    handle, nmode_A_in, extent_A_in, strides, modes_A_in, data_type
)
desc_tensor_B_in = cutn.create_tensor_descriptor(
    handle, nmode_B_in, extent_B_in, strides, modes_B_in, data_type
)
desc_tensor_G_in = cutn.create_tensor_descriptor(
    handle, nmode_G_in, extent_G_in, strides, modes_G_in, data_type
)

desc_tensor_A_out = cutn.create_tensor_descriptor(
    handle, nmode_A_out, extent_A_out, strides, modes_A_out, data_type
)
desc_tensor_B_out = cutn.create_tensor_descriptor(
    handle, nmode_B_out, extent_B_out, strides, modes_B_out, data_type
)
...
absCutoff_dtype = cutn.tensor_svd_config_get_attribute_dtype(
...
cutn.tensor_svd_config_set_attribute(
    ...
    svd_config,
    cutn.TensorSVDConfigAttribute.ABS_CUTOFF,
    absCutoff.ctypes.data,
    absCutoff.dtype.itemsize,
)

relCutoff_dtype = cutn.tensor_svd_config_get_attribute_dtype(
...
cutn.tensor_svd_config_set_attribute(
    ...
    svd_config,
    cutn.TensorSVDConfigAttribute.REL_CUTOFF,
    relCutoff.ctypes.data,
    relCutoff.dtype.itemsize,
)
...
full_extent_dtype = cutn.tensor_svd_info_get_attribute_dtype(
...
cutn.tensor_svd_info_get_attribute(
    ...
    full_extent.itemsize,
)
full_extent = int(full_extent)

reduced_extent_dtype = cutn.tensor_svd_info_get_attribute_dtype(
...
print(
    f"For fixed extent truncation of {shared_extent_out}, an absolute cutoff value of {float(absCutoff)}, and a relative cutoff value of {float(relCutoff)}, full extent {full_extent} is reduced to {reduced_extent}"
)
...
if reduced_extent != shared_extent_out:
    extent_A_out_reduced, strides_A_out = cutn.get_tensor_details(
        handle, desc_tensor_A_out
    )[2:]
    extent_B_out_reduced, strides_B_out = cutn.get_tensor_details(
        handle, desc_tensor_B_out
    )[2:]
    ...
    A_out_d = cp.ndarray(
        extent_A_out_reduced,
        dtype=np.float32,
        memptr=A_out_d.data,
        strides=strides_A_out,
    )
    S_out_d = cp.ndarray(
        reduced_extent, dtype=np.float32, memptr=S_out_d.data, order="F"
    )
    B_out_d = cp.ndarray(
        extent_B_out_reduced,
        dtype=np.float32,
        memptr=B_out_d.data,
        strides=strides_B_out,
    )
