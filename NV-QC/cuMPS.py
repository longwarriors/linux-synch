...
# https://github.com/NVIDIA/cuQuantum/blob/main/python/samples/cutensornet/approxTN
import itertools
...
class MPSHelper:
    ...
    def __init__(
        ...
        for i in range(self.num_sites):
            ...
            desc_tensor = cutn.create_tensor_descriptor(
                self.handle, 3, extent, 0, modes, self.data_type
            )
    ...
    def get_tensor_modes(self, site):
        ...
        return (
            self.virtual_modes[site],
            self.phys_modes[site],
            self.virtual_modes[site + 1],
        )
    ...
    def set_svd_config(self, abs_cutoff, rel_cutoff, renorm, partition):
        ...
        if partition != cutn.TensorSVDPartition.UV_EQUAL:
            raise NotImplementedError(
                "this basic example expects partition to be cutensornet.TensorSVDPartition.UV_EQUAL"
            )

        svd_config_attributes = [
            cutn.TensorSVDConfigAttribute.ABS_CUTOFF,
        ...
        for (attr, value) in zip(
            svd_config_attributes, [abs_cutoff, rel_cutoff, renorm, partition]
        ):
    ...
    def compute_max_workspace_sizes(self):
        ...
        desc_tensor_in_A = cutn.create_tensor_descriptor(
            self.handle, 3, max_extents_AB, 0, modes_in_A, self.data_type
        )
        desc_tensor_in_B = cutn.create_tensor_descriptor(
            self.handle, 3, max_extents_AB, 0, modes_in_B, self.data_type
        )
        desc_tensor_in_G = cutn.create_tensor_descriptor(
            self.handle, 4, extents_in_G, 0, modes_in_G, self.data_type
        )
        desc_tensor_out_A = cutn.create_tensor_descriptor(
            self.handle, 3, max_extents_AB, 0, modes_out_A, self.data_type
        )
        desc_tensor_out_B = cutn.create_tensor_descriptor(
            self.handle, 3, max_extents_AB, 0, modes_out_B, self.data_type
        )

        cutn.workspace_compute_gate_split_sizes(
            self.handle,
            desc_tensor_in_A,
            desc_tensor_in_B,
            desc_tensor_in_G,
        ...
        workspace_size = cutn.workspace_get_memory_size(
            self.handle,
            self.work_desc,
    ...
    def apply_gate(self, site_A, site_B, gate, verbose, stream):
        ...
        desc_tensor_in_G = cutn.create_tensor_descriptor(
            self.handle, 4, extent_G, 0, modes_G, self.data_type
        )
        ...
        combined_extent_left = (
            min(left_extent_A, extent_AB_in * self.phys_extent) * self.phys_extent
        )
        combined_extent_right = (
            min(right_extent_B, extent_AB_in * self.phys_extent) * self.phys_extent
        )
        extent_Aout_B = min(
            combined_extent_left, combined_extent_right, self.max_virtual_extent
        )
        ...
        modes_out_A = (
            self.virtual_modes[site_A],
            phys_mode_out_A,
            self.virtual_modes[site_A + 1],
        )
        modes_out_B = (
            self.virtual_modes[site_B],
            phys_mode_out_B,
            self.virtual_modes[site_B + 1],
        )

        desc_tensor_out_A = cutn.create_tensor_descriptor(
            self.handle, 3, extent_out_A, 0, modes_out_A, self.data_type
        )
        desc_tensor_out_B = cutn.create_tensor_descriptor(
            self.handle, 3, extent_out_B, 0, modes_out_B, self.data_type
        )

        cutn.gate_split(
            self.handle,
            desc_tensor_in_A,
            tensor_in_A.data.ptr,
            desc_tensor_in_B,
            tensor_in_B.data.ptr,
            desc_tensor_in_G,
            gate.data.ptr,
            desc_tensor_out_A,
            tensor_out_A.data.ptr,
            0,  # we factorize singular values equally onto output A and B.
            desc_tensor_out_B,
            tensor_out_B.data.ptr,
            self.gate_algo,
            self.svd_config,
            self.compute_type,
            self.svd_info,
        ...
        if verbose:
            full_extent = np.array(
                [0],
                dtype=cutn.tensor_svd_info_get_attribute_dtype(
                    cutn.TensorSVDInfoAttribute.FULL_EXTENT
                ),
            )
            reduced_extent = np.array(
                [0],
                dtype=cutn.tensor_svd_info_get_attribute_dtype(
                    cutn.TensorSVDInfoAttribute.REDUCED_EXTENT
                ),
            )
            discarded_weight = np.array(
                [0],
                dtype=cutn.tensor_svd_info_get_attribute_dtype(
            ...
            cutn.tensor_svd_info_get_attribute(
                ...
                self.svd_info,
                cutn.TensorSVDInfoAttribute.FULL_EXTENT,
                full_extent.ctypes.data,
                full_extent.dtype.itemsize,
            )
            cutn.tensor_svd_info_get_attribute(
                self.handle,
                self.svd_info,
                cutn.TensorSVDInfoAttribute.REDUCED_EXTENT,
                reduced_extent.ctypes.data,
                reduced_extent.dtype.itemsize,
            )
            cutn.tensor_svd_info_get_attribute(
                self.handle,
                self.svd_info,
                ...
                discarded_weight.dtype.itemsize,
            )

            print(
                "Virtual bond truncated from {0} to {1} with a discarded weight of {2:.6f}".format(
                    full_extent[0], reduced_extent[0], discarded_weight[0]
                )
            )
        ...
        extent_out_A, strides_out_A = cutn.get_tensor_details(
            self.handle, desc_tensor_out_A
        )[2:]
        extent_out_B, strides_out_B = cutn.get_tensor_details(
            self.handle, desc_tensor_out_B
        )[2:]
        ...
        if extent_out_A[2] != extent_Aout_B:
            ...
            tensor_out_A = cp.ndarray(
                extent_out_A,
                dtype=tensor_out_A.dtype,
                memptr=tensor_out_A.data,
                strides=strides_out_A,
            )
            tensor_out_B = cp.ndarray(
                extent_out_B,
                dtype=tensor_out_B.dtype,
                memptr=tensor_out_B.data,
                strides=strides_out_B,
            )
...
if __name__ == "__main__":
    ...
    worksize = free_mem * 0.7
    required_workspace_size = mps_helper.compute_max_workspace_sizes()
    work = cp.cuda.alloc(worksize)
    print(
        f"Maximal workspace size requried: {required_workspace_size / 1024 ** 3:.3f} GB"
    )
    ...
    for i in range(num_layers):
        ...
        for j in range(start_site, num_sites - 1, 2):
            ...
            gate = cp.random.random([phys_extent,] * 4) + 1.0j * cp.random.random(
                [
                    phys_extent,
                ]
                * 4
